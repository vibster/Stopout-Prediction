...
def seperate_train_test(file_base):
	...
	with open(in_file, 'r') as csv_file:
		...
		next(reader)
		for row in reader:
			data.append(map(float,row)[1:len(row)])
...
def runTestTrainSeparation(course_name):
	print "Starting train/test separation"
	file_base = course_name+"_processed"
	seperate_train_test(file_base)
	print "End train/test separation"
