...
class Course:
    ...
    def annotate_train_test(self,threshold):
        #number of user/week/feature_id rows

        #depends on not having redundant data from previous extractions
        get_length = '''select count(*)
        FROM
        ...
        ''' % (self.name,self.earliest_date, self.latest_date,
               utils.convert_list_to_str(list(self.features)),
               utils.convert_list_to_str(list(self.weeks)))


        cursor = self.conn.cursor()
        cursor.execute(get_length)
        data_length = cursor.fetchone()
        data_length = int(data_length[0])
        cursor.close()
        #this is different than value calculated in separate_traintest,
        #it's multiplied by the number of features, because the sql includes that as a
        # column, instead of the 2-dimensional csv file
        num_train = int(threshold*data_length/len(self.weeks))*len(self.weeks)
        #make sure it's a multiple of the number of features,
        #so each datum in training set is a full set of features
        num_train -= (num_train % len(self.features))
        num_test = data_length - num_train

        cursor = self.conn.cursor()
        annotate_train = '''
        UPDATE `%s`.user_longitudinal_feature_values
        SET train_data = 1
        WHERE date_of_extraction >= '%s'
        AND
        date_of_extraction <= '%s'
